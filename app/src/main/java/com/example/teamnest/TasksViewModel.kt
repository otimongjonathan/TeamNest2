...
class TasksViewModel : ViewModel() {
    ...
    fun startListeningMyTasks() {
        ...
        myTasksListener = db.collection("tasks")
            ...
            .addSnapshotListener { snapshot, _ ->
                myTasks.clear()
                snapshot?.documents?.mapNotNull { it.toObject(Task::class.java) }?.let { myTasks.addAll(it) }
            }
    }
    ...
    fun startListeningLeadGroups() {
        ...
        leadGroupsListener = db.collection("groups")
            ...
            .addSnapshotListener { snapshot, _ ->
                leadGroups.clear()
                snapshot?.documents?.mapNotNull { it.toObject(Group::class.java) }?.let { leadGroups.addAll(it) }
            }
    }
    ...
    fun listenToGroupTasks(groupId: String) {
        ...
        groupTasksListener = db.collection("tasks")
            ...
            .addSnapshotListener { snapshot, _ ->
                ...
                snapshot?.documents?.mapNotNull { it.toObject(Task::class.java) }?.let { selectedGroupTasks.addAll(it) }
            }
    }
